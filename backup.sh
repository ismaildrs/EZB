...
B2_APPLICATION_KEY_ID="002d2b3c09edde70000000002"
B2_APPLICATION_KEY="K002gXaiXXQtEZM6LE6ZqJlh0fBG9vg"
BUCKET_NAME="BackupProjet"
...
show_help() {
    ...
    echo "-r, --restore        Restaurer les fichiers à partir de la sauvegarde"
    echo "-l, --log            Log file"
    echo "-fo, --forked        Fork"
    echo "-t, --thread         Utiliser des threads"
...
# Function to compress folder in a separate thread
compress_folder_thread() {
    local source_folder="$1"
    local destination="$2"

    echo "Compression en cours dans un thread séparé..."
    tar -czf "$destination" "$source_folder"
    echo "Compression terminée dans un thread séparé."
}
